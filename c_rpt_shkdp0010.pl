/opt/etl/prd/etl/APP/ADW/C_RPT_SHKDP/bin> cat c_rpt_shkdp0010.pl
######################################################
...
sub initParam{
        ...
        $touch_date_d = &UnixDate("${etlvar::TXDATE}", "%Y%m%d");
        $touch_date_m = &UnixDate(DateCalc("${etlvar::TXDATE}", "+ 1 months", \$err), "%Y%m%d");
        $update_job_status_date_d = &UnixDate(DateCalc("${etlvar::TXDATE}", "- 1 days", \$err), "%Y-%m-%d");
        $update_job_status_date_m = &UnixDate(DateCalc("${etlvar::TXDATE}", "- 1 months", \$err), "%Y-%m-%d");
        $update_job_status_date = &UnixDate(DateCalc("${etlvar::TXDATE}", "- 0 months", \$err), "%Y-%m-%d");
...
delete from ${etlvar::ETLDB}.ETL_RECEIVED_FILE where ETL_JOB='X_SHKDP_CE';
delete from ${etlvar::ETLDB}.ETL_RECEIVED_FILE where ETL_JOB='X_SHKDP_RULE_REF';
delete from ${etlvar::ETLDB}.ETL_RECEIVED_FILE where ETL_JOB='X_SHKDP_DICT_REF';

update ${etlvar::ETLDB}.ETL_JOB set LAST_TXDATE=date'$update_job_status_date_m',LAST_JOBSTATUS='Ready' where ETL_JOB='X_SHKDP_CE';
update ${etlvar::ETLDB}.ETL_JOB set LAST_TXDATE=date'$update_job_status_date',LAST_JOBSTATUS='Ready' where ETL_JOB='X_SHKDP_RULE_REF';
update ${etlvar::ETLDB}.ETL_JOB set LAST_TXDATE=date'$update_job_status_date_m',LAST_JOBSTATUS='Ready' where ETL_JOB='X_SHKDP_DICT_REF';
...
update ${etlvar::ETLDB}.ETL_SRC_FILE set LAST_PROCESS_START_TS=null where job_name='X_SHKDP_CE';
update ${etlvar::ETLDB}.ETL_SRC_FILE set LAST_PROCESS_START_TS=null where job_name='X_SHKDP_RULE_REF';
update ${etlvar::ETLDB}.ETL_SRC_FILE set LAST_PROCESS_START_TS=null where job_name='X_SHKDP_DICT_REF';
...
if ($ret == 0){
        system("touch /opt/etl/prd/etl/preprocess/USR/rerun/x_shkdp_ce.rerun.'$touch_date' ");
        system("touch /opt/etl/prd/etl/preprocess/USR/rerun/x_shkdp_rule_ref.rerun.'$touch_date' ");
        system("touch /opt/etl/prd/etl/preprocess/USR/rerun/x_shkdp_dict_ref.rerun.'$touch_date' ");
