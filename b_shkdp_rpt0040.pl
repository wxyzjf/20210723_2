    ...
    ---- Deduplicate mkt code in this level---
      ...
      from
        (
            ...
            from mig_adw.shkdp_rule_ref ru           
        )r 
...
(
        ...
        ,case when upper(t.pr_final_status) in(upper('Accept Offer')) and t.af_plan_mkt_cd <>' '  and t.af_channel_effort ='Outbound' then 'HIT:OUTBOUND' 
              when upper(t.pr_final_status) in(upper('Accept Offer')) and t.af_plan_mkt_cd <>' ' and t.af_channel_effort ='Inbound' then 'HIT:INBOUND' 
                --- no pr ----
              when upper(t.pr_final_status) in(upper('Accept Offer')) and t.af_plan_mkt_cd <>' ' and t.af_channel_effort <> ' ' then 'HIT:'||upper(t.af_channel_effort)
              else ' '
         end cl_hit
...
select 
    ...
    ,af_plan_tariff
    ,shk_tier
from mig_adw.SHKDP_RPT_004C02_T;
commit;
