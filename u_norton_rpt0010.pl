/opt/etl/prd/etl/APP/RPT/U_NORTON_RPT/bin> cat u_norton_rpt0010.pl
####################################################
#   $Header: 
...
sub runSQLPLUS{
  ...
  #my $SQLCMD_FILE="${etlvar::AUTO_GEN_TEMP_PATH}raw_data_report_sqlcmd.sql";
  my $SQLCMD_FILE="/opt/etl/prd/etl/APP/RPT/U_NORTON_RPT/bin/raw_data_report_sqlcmd.sql";
...
--SPOOL '/opt/etl/prd/etl/APP/RPT/U_NORTON_RPT/bin/raw_data_report.csv';

SELECT 'CUST_NUM,SUBR_NUM,BILL_SERV_CD,START_DATE,END_DATE,NUM_ID,USER_ID,CAMP_ID,PRODUCT_SKU,SUBSCRIP_START_DATE,SUBSCRIP_CANCEL_DATE,SUBSCRIP_CUR_MON,DEMO_UNIT,CANCEL_UNIT,BILLABLE_UNIT'
FROM DUAL;
...
select  
  ...
  ||','||to_char(SUBSCRIP_CUR_MON,'YYYYMM')
  ||','||DEMO_UNIT
  ||','||CANCEL_UNIT
  ||','||case when DEMO_UNIT = 'Y' or CANCEL_UNIT = 'Y' then 'N' else 'Y' end
from MIG_ADW.B_NORTON_RPT_001;
...
sub initParam{
   ...
   # $RUN_DATE = &UnixDate(DateCalc("${etlvar::TXDATE}", "- 0 days", \$err), "%Y-%m-%d");
    ...
    if ($ENV eq "DEV"){
        ...
        $OUTPUT_FILE_NAME = "raw_data_report.csv";
    }
    else
    {
        ##  PRODUCTION  ##
