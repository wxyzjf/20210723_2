...
define opt_date=to_date('$etlvar::TXDATE','YYYY-MM-DD');
...
prompt 'Step SHKDP_RPT_002C01_T: [ Get PR module v2 ] ';
------------ PR --------
...
from MIG_ADW.SHKDP_RPT_001_T t
    ...
    ,(
      select r.dict_val
            ,r.batch_id
      from mig_adw.shkdp_dict_ref r
      where r.dict_grp ='PRG_ID:PR-PRG_LST'
      and &opt_date between r.eff_start_date and r.eff_end_date) pl
where t.cust_num = pr.cust_num
  and t.subr_num = pr.subr_num
  and pr.lst_mod_date between  t.contact_start_date and &opt_date
  and pr.program_id = pl.dict_val
...
left outer join PRD_BIZ_SUMM_VW.VW_SUBR_LD_ST_DATE sl
  ...
  ,(
        select batch_id,dict_val from mig_adw.SHKDP_DICT_REF where dict_grp ='MKT_CD:BF-FUP'
        and &opt_date between eff_start_date and eff_end_date          
    )m
where 
    pg.mkt_cd = m.dict_val
...
select     
    ...
    ,substr(nvl(prb.other_list ,' '),1,1500)                    as bf_other_lst
    ,substr(nvl(prb.json_rmk,' '),1,1500)                       as bf_json_rmk
from    mig_adw.SHKDP_RPT_001_T t
